...
echo ""
echo "Browser URL: http://$(curl -fsS --max-time 3 https://checkip.amazonaws.com/ 2>/dev/null || echo PUBLIC_IP):7860"
echo "If curl above shows 200 OK but the browser cannot connect → AWS Security Group:"
echo "  EC2 → instance → Security → Security groups → Edit inbound rules → add TCP 7860 from 0.0.0.0/0 (demo) or your IP."
